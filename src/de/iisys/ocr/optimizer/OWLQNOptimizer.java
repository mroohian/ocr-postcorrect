 ...
 */
public class OWLQNOptimizer implements IOptimizer {
    ...
    public OWLQNOptimizer(IFunction function) {
        this(function, 0.0);
    }

    public OWLQNOptimizer(IFunction function, double l1wt) {
        ...
        int numParameters = optimizable.getNumParams();
    ...
    public boolean optimize(int numIterations) {
        System.err.println("Entering OWL-BFGS.optimize(). L1 weight=" + l1Weight
                + " Initial Value=" + value);
        ...
        for (int iter = 0; iter < numIterations; iter++) {
            ...
            if (checkValueTerminationCondition()) {
                System.err.println("Exiting OWL-BFGS on termination #1:");
                System.err.println("value difference below tolerance (oldValue: "
            ...
            if (checkGradientTerminationCondition()) {
                System.err.println("Exiting OWL-BFGS on termination #2:");
                System.err.println("gradient=" + MatrixOps.twoNorm(grad) + " < "
                        + gradientTolerance);
                converged = true;
                return true;
            }
            ...
            if (iterations > maxIterations) {
                System.err.println("Too many iterations in OWL-BFGS. "
    ...
    private double evalL1() {
        ...
        System.err.println("getValue() (" + optName + ".getValue() = " + val
    ...
    private void mapDirByInverseHessian(double yDotY) {
        ...
        System.err.println("Direction multiplier = " + scalar);
    ...
    private double shift() {
        ...
        if (rho < 0) {
            throw new RuntimeException("rho = " + rho + " < 0: "
    ...
    private void backTrackingLineSearch() {
        ...
        double backoff = 0.5;
        if (iterations == 0) {
            double normDir = Math.sqrt(MatrixOps.dotProduct(direction, direction));
            alpha = 1.0 / normDir;
            backoff = 0.1;
        }
        ...
        System.err.println("*** Starting line search iter=" + iterations);
        System.err.println("iter[" + iterations + "] Value at start of line search = "
                + value);
        ...
        while (true) {
            ...
            System.err.println("iter[" + iterations + "] Using alpha = " + alpha
