...
public class BackTrackLineSearch implements ILineOptimizer {
    IFunction function;
    private final boolean mLogFine = false;

    public BackTrackLineSearch (IFunction optimizable) {
    ...
    public double optimize (double[] line, double initialStep)
    {
        ...
        double f, fold, f2;
        g = new double[function.getNumParams()]; // gradient
        x = new double[function.getNumParams()]; // parameters
        oldParameters = new double[function.getNumParams()];
        ...
        if (mLogFine) {
            System.err.println("ENTERING BACKTRACK\n");
            System.err.println("Entering BackTrackLnSrch, value="+fold+",\ndirection.oneNorm:"
        ...
        if(sum > stpmax) {
            System.err.println("attempted step too big. scaling: sum="+sum+
                    ", stpmax="+stpmax);
        ...
        if (slope<0)
            throw new RuntimeException ("Slope = " + slope + " is negative");
        if (slope == 0)
            throw new RuntimeException ("Slope = " + slope + " is zero");
        ...
        for(iteration=0; iteration < maxIterations; iteration++) {
            ...
            System.err.println("BackTrack loop iteration "+iteration+": alam="+
                    alam+" oldAlam="+oldAlam);
            System.err.println("before step, x.1norm: " + MatrixOps.oneNorm(x) +
                    "\nalam: " + alam + "\noldAlam: " + oldAlam);
            assert(alam != oldAlam) : "alam == oldAlam";
            MatrixOps.plusEquals(x, line, alam - oldAlam); // step
            System.err.println("after step, x.1norm: " + MatrixOps.oneNorm(x));
...
//				if ((alam < alamin)) {
                ...
                f = function.getValue();
                System.err.println("EXITING BACKTRACK: Jump too small (alamin="+alamin+"). Exiting and using xold. Value="+f);
                return 0.0;
            }
            ...
            System.err.println("value="+f);

            // sufficient function increase (Wolf condition)
            if(f >= fold+ALF*alam*slope) {

                System.err.println("EXITING BACKTRACK: value="+f);
            ...
            // jumped to unstable territory, then scale down jump
                ...
                System.err.println("Value is infinite after jump " + oldAlam + ". f="+f+", f2="+f2+". Scaling back step size...");
                tmplam = .2 * alam;
                if(alam < alamin) { //convergence on delta x
                    function.setParameters(oldParameters);
                    f = function.getValue();
                    System.err.println("EXITING BACKTRACK: Jump too small. Exiting and using xold. Value="+f);
                    return 0.0;
                }
            }
            else { // backtrack
            ...
            alam2 = alam;
            f2 = f;
            System.err.println("tmplam:"+tmplam);
